...
interface ExcluirContratoButtonProps {
  contratoId: string
  numeroContrato: string
}

export function ExcluirContratoButton({ contratoId, numeroContrato }: ExcluirContratoButtonProps) {
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      const result = await deleteContrato(contratoId, password)
      if (result?.error) {
        toast.error(result.error)
      } else {
        toast.success(`Contrato Nº ${numeroContrato} excluído com sucesso!`)
        setPassword('')
        setOpen(false)
      }
    })
      ...
      <DialogContent className="sm:max-w-[425px] border-red-500/20 bg-card">
        <DialogHeader>
          ...
          <DialogDescription className="text-muted-foreground text-xs leading-relaxed mt-2">
            Tem certeza que deseja excluir permanentemente o contrato <strong className="text-foreground">Nº {numeroContrato}</strong>?
            <br /><br />
            Esta ação é **irreversível** e apagará todo o histórico desse contrato.
          </DialogDescription>
        </DialogHeader>
